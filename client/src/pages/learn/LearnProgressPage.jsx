export default function LearnProgressPage() {
  const globalProgress = 62;

  const subjectProgress = [
    { matiere: 'Mathématiques', val: 68, color: '#7C6FFF' },
    { matiere: 'SVT', val: 55, color: '#00D4AA' },
    { matiere: 'Physique-Chimie', val: 40, color: '#F59E0B' },
    { matiere: 'Anglais', val: 80, color: '#3B82F6' }
  ];
    ...
    <div style={{ maxWidth: '1000px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div style={{ ...cardStyle, background: '#1A1A1A', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ margin: 0, color: '#94A3B8', fontSize: '1.1rem' }}>Examen préparé : BAC Série D</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
        ...
        {/* OBJECTIFS EN COURS (Point 13.2) */}
        <div style={cardStyle}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            <h2 style={{ margin: 0, fontSize: '1.3rem', fontWeight: 800 }}>Objectifs en cours</h2>
            <button style={{ background: '#F5F4EF', border: 'none', padding: '0.4rem 1rem', borderRadius: '2rem', cursor: 'pointer', fontWeight: 600 }}>+ Nouveau</button>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {currentGoals.map((goal, i) => (
              <div key={i} style={{ background: '#F9F9F8', padding: '1.5rem', borderRadius: '1rem', border: '1px solid #E5E5E2' }}>
                <h3 style={{ margin: '0 0 0.5rem 0', fontSize: '1.1rem' }}>{goal.title}</h3>
                <p style={{ margin: '0 0 1rem 0', fontSize: '0.85rem', color: '#6E6E6B' }}>{goal.date}</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  <div style={{ flex: 1, height: '8px', background: '#E5E5E2', borderRadius: '4px', overflow: 'hidden' }}>
                    <div style={{ width: `${goal.progress}%`, height: '100%', background: '#1A1A1A', borderRadius: '4px' }}></div>
                  </div>
                  <span style={{ fontWeight: 700, fontSize: '0.9rem' }}>{goal.progress}%</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* HISTORIQUE D'ACTIVITÉS (Point 13.3) */}
      <div style={cardStyle}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {recentActivities.map((act, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', background: '#FAFAFA', borderRadius: '0.75rem', border: '1px solid #F0F0EE' }}>
              <div>
                <div style={{ fontWeight: 600, color: '#1A1A1A', marginBottom: '0.3rem' }}>{act.action}</div>
                <div style={{ fontSize: '0.85rem', color: '#6E6E6B' }}>{act.time}</div>
              </div>
